...
std::vector<color> loadImgFromPng(const std::string& filename, int2& dims){
  ...
  uint64_t bit_idx = 0;
  ERROR("this bit_idx may end up being too small, should have some check to prevent overflow"); // best fix for this is probably take mod of data array size when incrementing
